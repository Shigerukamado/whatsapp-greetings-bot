...
async function getUpcomingEvents() {
    ...
    for (let i = 0; i <= 30; i++) {
        ...
        const fullDate = d.toLocaleDateString('en-GB', { day: 'numeric', month: 'long' });
        ...
        if (holidays[dateKey]) {
            ...
            const title = msg.split('!')[0].replace(/[🎉🌸❤️🌷👨‍💼🎄🎁]/g, '').trim();
            events.push({
                date: fullDate,
                daysAway: i,
                type: 'holiday',
...
app.get('/', async (req, res) => {
    ...
    if (holidays[todayDate]) {
        ...
        todayEvent = msg.split('!')[0].replace(/[🎉🌸❤️🌷👨‍💼🎄🎁]/g, '').trim();
    }
    const birthdaysToday = contacts.filter(c => (c.birthday || '').trim() === todayDate);
    if (birthdaysToday.length > 0) {
        todayEvent = `🎂 ${birthdaysToday.map(c => (c.name || '').trim()).join(', ')}'s Birthday`;
    }

    const statusColor = botStatus === 'connected' ? '#22c55e' : botStatus === 'connecting' ? '#f59e0b' : '#ef4444';
    const statusText = botStatus === 'connected' ? '✅ Connected' : botStatus === 'connecting' ? '⏳ Connecting...' : '❌ Disconnected';
